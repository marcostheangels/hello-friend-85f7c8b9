...
function EventoCard({ evento }: { evento: Evento }) {
  ...
  return (
    <article
      className={`group relative overflow-hidden rounded-2xl border bg-card p-6 transition-all hover:-translate-y-1 ${
        destaque
          ? "border-primary/70 shadow-[0_20px_60px_-25px_rgba(220,38,38,0.6)] md:col-span-2 lg:col-span-2"
          : "border-border hover:border-primary/50"
      }`}
    >
      {destaque && (
        <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-red-700 via-red-500 to-amber-400" />
      )}

      <div className={destaque ? "grid gap-7 md:grid-cols-[auto_1fr]" : ""}>
        <div
          className={`flex shrink-0 flex-col items-center justify-center rounded-xl text-primary-foreground ${
            destaque ? "h-24 w-24 md:h-28 md:w-28" : "h-16 w-16"
          }`}
        ...
        <div>
          ...
          <h3 className={`mt-4 font-display tracking-wide text-foreground ${destaque ? "text-3xl md:text-4xl" : "text-2xl"}`}>
            {evento.titulo}
          </h3>
          ...
          {destaque && (
            <div className="mt-6 grid gap-3 rounded-xl border border-primary/20 bg-primary/5 p-4 sm:grid-cols-3">
              {[
                ["Forro Xinela", "Atração"],
                ["Marcelo Moreno", "Atração"],
                ["Cid Souza", "Atração"],
              ].map(([nome, tipo]) => (
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            {destaque && (
              <Button asChild variant="outline" className="font-semibold">
                <a href="https://wa.me/5538988429992" target="_blank" rel="noreferrer">
                  <MessageCircle className="mr-2 h-4 w-4" /> Informações pelo WhatsApp
                </a>
              </Button>
            )}
          </div>
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main id="topo">
        <section className="relative isolate overflow-hidden">
          ...
          <div className="relative mx-auto max-w-6xl px-5 py-24 md:py-36">
            ...
            <p className="mt-5 max-w-xl text-lg text-muted-foreground">
              Shows, festas, cultura, esportes e gastronomia. A agenda completa de Montes Claros atualizada toda semana.
            </p>
        ...
        <section id="eventos" className="mx-auto max-w-6xl px-5 py-20">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-muted-foreground">Confira o destaque desta semana e descubra o que fazer.</p>
            </div>
          ...
          {lista.length === 0 && (
            <p className="mt-12 text-center text-muted-foreground">Nenhum evento encontrado. Tente outra busca.</p>
          )}
        </section>
        ...
        <section id="anuncie" className="border-y border-border bg-card/50">
          <div className="mx-auto grid max-w-6xl gap-10 px-5 py-20 md:grid-cols-2 md:items-center">
            <div>
              <h2 className="font-display text-4xl tracking-wide md:text-5xl">Divulgue seu evento para toda a cidade</h2>
              <p className="mt-4 text-muted-foreground">
                Planos de destaque na home, posts nas redes e alcance direto com o público que sai em Montes Claros.
              </p>
      ...
      <footer id="contato" className="mx-auto max-w-6xl px-5 py-14">
        <div className="flex flex-col justify-between gap-8 md:flex-row">
          <div>
            <p className="font-display text-2xl tracking-widest">AGENDA <span className="text-primary">MOC</span></p>
            <p className="mt-2 max-w-sm text-sm text-muted-foreground">A agenda de eventos de Montes Claros, Norte de Minas. Feito por quem vive a cidade.</p>
          </div>
          <ul className="space-y-2 text-sm text-muted-foreground">
            <li className="flex items-center gap-2"><Mail className="h-4 w-4 text-primary" aria-hidden /> contato@agendamoc.com.br</li>
            <li className="flex items-center gap-2"><Phone className="h-4 w-4 text-primary" aria-hidden /> (38) 90000-0000</li>
            <li className="flex items-center gap-2"><Instagram className="h-4 w-4 text-primary" aria-hidden /> @agendamoc</li>
          </ul>
        </div>
